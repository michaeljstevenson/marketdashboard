...
* **IPO Aftermarket Performance** (2026-09-14, investigated this session) — skipped after checking the data, not abandoned early. `/ipo-activity.html`'s only data source is Jay Ritter's *aggregate, year-level* IPO statistics (counts, average first-day return, etc.) — it has no list of individual IPO companies/tickers/dates to join price history against. Alpha Vantage's `IPO_CALENDAR` is forward-looking (upcoming IPOs only) and can't supply historical completed-IPO tickers either. Building this properly would need a real historical IPO-by-ticker dataset (company, ticker, IPO date) from a source not yet identified — worth a future session's dedicated research pass before attempting again, rather than another quick check.
* **Short Interest Tracker** — not attempted this session (ran out of time/scope, not a confirmed dead end). Alpha Vantage has no short-interest endpoint; FINRA publishes a free semi-monthly full-market short-interest file that's the most promising lead, but this session didn't get far enough to confirm its format/reliability for a scheduled pull. Worth a real attempt next run rather than treating this note as a "skip."
