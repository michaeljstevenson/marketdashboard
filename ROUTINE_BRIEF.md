...
* **Earnings Surprise History (beat/miss rates by sector)** — built (Alpha Vantage `EARNINGS`), tested, and initially shipped in `nightly/2026-09-13-earnings-surprise`, but removed after the fact as a duplicate of the already-built Earnings Revisions page and dropped from the stack. A future run should not re-add it without first checking whether that duplication concern still applies.
* **Dividend Growth Screener** — built (Alpha Vantage `DIVIDENDS`), tested, and initially shipped as part of `nightly/2026-09-13-dividend-growth`, but removed after the fact per explicit request, flagged as duplicate content (the specific overlap wasn't recorded at removal time — worth double-checking against the rest of the site's Equities section, e.g. Share Count Trends's shareholder-return angle, before rebuilding). A future run should not re-add it without first confirming whether that duplication concern still applies.
