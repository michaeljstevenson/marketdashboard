...
* **Earnings Surprise History** (2026-09-13, `nightly/2026-09-13-earnings-surprise`, stacked on top of the Small Cap vs. Large Cap Spread branch/PR above — merge that one first, or merge this PR as-is since it carries both diffs) — `/earnings-surprise.html`. Reported-vs-estimated EPS (Alpha Vantage `EARNINGS`, which computes the surprise/surprise% itself) across the full S&P 500, trailing 8 quarters: beat rate by sector, a market-wide beat-rate trend by calendar quarter, a two-proportion z-test for whether beating (or missing) last quarter predicts beating this quarter ("beat persistence"), longest active beat/miss streak leaderboards, and biggest beats/misses this reporting season. Distinct from the already-built Earnings Revisions page: that page is forward-looking analyst *estimate revisions* (`EARNINGS_ESTIMATES`); this one is backward-looking actual *reported-vs-estimate* results (`EARNINGS`) — different Alpha Vantage endpoint, different question. Backend: weekly `scheduled-earnings-surprise-background.js` (Saturday 10:50 UTC, after the insider-transactions sweep), reusing the Sector Beeswarm page's `meta.json` for company name/sector. No history-accumulation blob needed (unlike Earnings Revisions) since `EARNINGS` returns each company's own multi-quarter history in one call. Tested locally the same way as the Small Cap page (synthetic dataset with a deliberately-correlated beat/miss sequence to exercise the persistence test's significant-result path, served through a local stub API, driven with Playwright in dark/light/mobile) — no bugs found this time.
* **Share Count Trends (dilution vs. buybacks)** (2026-09-13, `nightly/2026-09-13-share-count-trends`, stacked on top of the Earnings Surprise History branch/PR above — same stacking rationale, merge earlier PRs first or merge this one as-is) — `/share-count-trends.html`. Reads buybacks vs. dilution directly off quarterly `commonStockSharesOutstanding` (Alpha Vantage `BALANCE_SHEET`) at 1/3/5-year lookbacks across the full S&P 500 — deliberately not sourced from separate buyback-announcement or dollar-value data, since the share count itself already nets out every buyback, issuance, and stock-based-comp dilution. Sector aggregates, a year-over-year persistence check (Pearson+Spearman, same non-overlapping-annual construction as `/factor-analysis`'s momentum test — does last year's buyback pace predict this year's?), buyback/dilution streak leaderboards, and a full sortable company table. Backend: weekly `scheduled-share-count-background.js` (Saturday 11:05 UTC, after the earnings-surprise sweep), reusing Sector Beeswarm's `meta.json` for company name/sector. Tested locally the same way as the other two pages this session (synthetic dataset with a deliberately-correlated year-over-year buyback signal, served through a local stub API, driven with Playwright in dark/light/mobile) — no bugs found.
