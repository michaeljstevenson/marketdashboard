...
* **International vs. US Relative Performance** (2026-09-15, `claude/bold-clarke-jzkkd9`) — `/international-vs-us.html`. Relative strength of developed-market (EFA) and emerging-market (EEM) equities against the US (SPY), indexed since UUP's Feb-2007 inception: a long-run ratio chart, trailing-return ladder, rolling 12-month relative-return oscillator, average monthly relative return by dollar regime (Rising/Flat/Falling, classified off UUP's trailing 3-month % change — same construction as Small Cap vs. Large Cap's Fed-regime split), and a Pearson+Spearman regression of monthly relative return (EFA-vs-SPY and EEM-vs-SPY separately) against that month's own % change in UUP — a direct test of the "a strong dollar is a headwind for international equities" thesis. Backend: new daily `scheduled-intl-background.js` (weekdays 21:20 UTC, in the gap between `scheduled-smallcap` and `scheduled-beeswarm-daily`) → Netlify Blobs (`intl-vs-us` store) → `international-vs-us.js`. Data: Alpha Vantage `TIME_SERIES_DAILY_ADJUSTED` only (SPY, EFA, EEM, UUP — 4 calls/day, no macro-series call needed since UUP's own price series doubles as the dollar-strength measure). Tested locally: synthetic dataset (real UUP-inception date range, a deliberately dollar-loaded random walk for EFA/EEM so the regression sign could be sanity-checked) served through a local stub API, page JS driven end-to-end with Playwright (chart-construction calls, stat/table/regression output, dark/light/mobile) — CDN access to the real Chart.js is blocked in this sandbox, so Chart.js itself was stubbed with a no-op recorder for the test run only; the shipped file is unchanged and uses the same CDN `<script>` tag as every other page on the site. No bugs found.
* **Stock Split Tracker** (2026-09-15, `claude/bold-clarke-jzkkd9`, same session as International vs. US above) — `/stock-splits.html`. Full S&P 500 sweep of Alpha Vantage's `SPLITS` endpoint (trailing 6 years), classified forward vs. reverse, with split frequency by year, a ratio-size distribution, a sector breakdown, and — the core analytical test — market-relative (vs. SPY) forward excess returns at 1M/3M/6M/1Y after each forward split's effective date, with a one-sample t-test of whether the average excess return differs from zero at each horizon (a direct test of the classic Ikenberry-style post-split-drift anomaly). Reverse splits are tracked and shown but deliberately excluded from the performance test (a distress signal, not a growth one — pooling them would muddy the result). Backend: new weekly `scheduled-splits-background.js` (Saturday 11:10 UTC, after `scheduled-share-count-background`) — a two-stage sweep: ~503 sequential `SPLITS` calls across the full universe (same 1050ms-pacing, 2-pass-retry pattern as the site's other full sweeps), then a smaller single-pass `TIME_SERIES_DAILY_ADJUSTED` (full history) sweep limited to just the tickers with an old-enough forward split to score a 1-year return, plus one SPY history call as the benchmark → Netlify Blobs (`stock-splits` store) → `stock-splits.js`. Company name/sector reused from Sector Beeswarm's weekly `meta.json`, same as this session's other page and the site's existing insider-transactions/share-count pages. Tested locally the same way as International vs. US above (synthetic dataset with a deliberate mild positive post-split drift signal to sanity-check the t-test math, including its sortable full event table) — no bugs found.
