...
// netlify.toml) that computes the full U.S. Market Sentiment Index —
// every factor score plus the composite history — once daily and writes
// it to Netlify Blobs for data.js to serve.
//
// This used to run live inside data.js on every single page load: 6+
// sequential Alpha Vantage calls (SPY, VIX, HYG, LQD, IWM, RSP) plus a
// News Sentiment call, on top of reading the breadth and put/call blobs.
// That meant every visitor paid for the full fetch-and-compute pipeline,
// and the response time (and Alpha Vantage call volume) scaled with
// traffic instead of staying flat. Moving it here, once a day, means
// data.js is now just a blob read — fast and free of any live upstream
// dependency — same pattern already used for Market Breadth and
// Put/Call (see scheduled-breadth-background.js and
// scheduled-putcall-background.js).
//
// Runs once daily after the close, staggered after both
// scheduled-breadth-background.js (needs its blob) and
// scheduled-putcall-background.js (needs its blob) have finished.
//
...
//   Equity Put/Call     — SPY's full-chain options put/call ratio, read from the
//                         putcall blob (scheduled-putcall-background.js).
...
//                         highs/lows, % above 200-day SMA), read from the breadth blob
//                         (scheduled-breadth-background.js; see breadth-constituents.js).
...
// the breadth specs above each need.
...
// still much shorter than the other factors', it's excluded from the
// composite history chart but still counts fully toward the composite
// score.
...
// Small-Cap Relative Strength: IWM (small-cap) vs. the S&P 500 — investors
...
const SMALLCAP_SPEC = {
  ...
  details:
    "This factor tracks the price ratio of IWM (a small-cap Russell 2000 fund) to the S&P 500 — small-caps carry more volatility and less balance-sheet cushion than large-caps, so investors chase them more aggressively when confident and rotate back to safer large-caps first when fear rises.\n\nThe ratio is rebased to start at 100 so it reads as a clean index. The score compares today's reading to its own 50-day average, ranked against its full history: the ratio running above trend suggests risk appetite for smaller, riskier names is increasing; running below trend suggests a flight to large-cap safety.",
};

// Equal-Weight Participation: RSP (equal-weight S&P 500) vs. the
// cap-weighted index — a breadth/complacency signal. When the equal-weight
// index keeps pace with or outpaces the cap-weighted one, gains are
// broad-based across the index rather than concentrated in a handful of
// mega-caps.
...
// excluded from the composite history chart, but still counts fully
...
const NEWSSENTIMENT_SPEC = {
  ...
  details:
    "This factor aggregates Alpha Vantage's News & Sentiment feed for SPY-tagged financial news articles, averaging each day's per-article sentiment score (roughly -1 very bearish to +1 very bullish) — an automated read on how the financial press is framing the market.\n\nThe score compares each day's average article sentiment to its own trailing 10-day average, ranked against the ~60 days of coverage this factor has usable history for. Because the News & Sentiment API returns a bounded recent feed rather than a bulk decades-long history, this factor — like Equity Put/Call — isn't included in the composite history chart below, but still counts fully toward the composite score.",
};
...
exports.handler = async () => {
  ...
  try {
    ...
    // factors that depend on it, rather than failing the whole run —
    // most factors here (credit, breadth internals, put/call) have
    // nothing to do with SPX/VIX and should still compute. Warnings are
    // collected and stored alongside the result instead of thrown,
    // unless every single factor ends up unavailable.
    const warnings = [];
    ...
    // volatility.js, but those only had 2-3 calls to space out. No
    // longer latency-sensitive now that this runs once daily in the
    // background rather than live on every page load.
    ...
    // NEWSSENTIMENT_SPEC) — but both still count fully toward the
    ...
    await store.setJSON(SENTIMENT_BLOB_KEY, data);
    console.log(`scheduled-sentiment-background: wrote composite=${composite}, ${components.length} components, ${history.length} history points`);

    return { statusCode: 200, body: JSON.stringify({ ok: true, composite, components: components.length }) };
  } catch (err) {
